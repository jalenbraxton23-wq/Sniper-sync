...
async function refreshNflOdds(env) {
  ...
  const games = allGames.filter((g) => {
    ...
    return Number.isFinite(t) && t >= now && t <= now + eightDaysMs;
  });
...
export default {
  ...
  async scheduled(event, env, ctx) {
    ...
    } else if (event.cron === "0 12 * * 0" || event.cron === "0 12 * * 1" || event.cron === "0 12 * * 3" ||
               event.cron === "0 12 * * 4" || event.cron === "0 12 * * 5" || event.cron === "0 12 * * 6") {
