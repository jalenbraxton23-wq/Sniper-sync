...
// D1 batches have a practical limit on statements per call -- Cloudflare's
// own guidance is to chunk into roughly 100-250 rows per batch rather than
// sending everything in one call. NFL in particular can easily produce
// several hundred rows in one refresh (6 markets x every player across a
// full week's slate), which is very likely what silently dropped most of
// its markets even though the refresh reported success -- the batch call
// itself was probably too large. MLB and WNBA have smaller datasets today,
// but chunking here protects all three as the season and data grow.
async function chunkedBatch(db, statements, chunkSize) {
...
// D1 rejects undefined and NaN outright (throws D1_TYPE_ERROR) -- these two
// helpers are the single choke point every numeric value passes through
// before it's ever stored, so no future NaN/undefined can reach a .bind()
// call again, no matter which stat or bookmaker is missing a field
function safeInt(v) {
...
// The Odds API returns decimal odds (e.g. 2.02), not American (+900/-380)
// like SportsGameOdds and the rest of the site use -- converts so both
// providers display consistently everywhere
...
const WNBA_MARKET_KEYS = {
  "player_points": "points",
  "player_rebounds": "rebounds",
  "player_assists": "assists",
};

const NFL_MARKET_KEYS = {
...
// pulls real NFL player prop odds from The Odds API. NFL's own odds
// endpoint returns the ENTIRE remaining season (dozens of games months
// out), unlike MLB/WNBA where the games list is naturally just what's
// upcoming soon -- fetching props for every game in that list would blow
// through the monthly request budget in one refresh, so this filters to
// only games within the next 8 days (this week's real slate) before
// spending any per-game requests on props
async function refreshNflOdds(env) {
  const apiKey = env.NFL_ODDS_API_KEY;
  if (!apiKey) {
    console.error("NFL_ODDS_API_KEY not set");
    return { ok: false, reason: "NFL_ODDS_API_KEY not set" };
  }
  const gamesRes = await fetch(
    "https://api.the-odds-api.com/v4/sports/americanfootball_nfl/odds/?apiKey=" + apiKey + "&regions=us&markets=h2h"
  );
  if (!gamesRes.ok) {
    console.error("The Odds API NFL games fetch failed", gamesRes.status);
    return { ok: false, reason: "games fetch failed, status " + gamesRes.status };
  }
  const allGames = await gamesRes.json();
  if (!Array.isArray(allGames)) return { ok: false, reason: "games response wasn't an array" };

  const now = Date.now();
  const eightDaysMs = 8 * 24 * 60 * 60 * 1000;
  // find the earliest upcoming game first, then take everything within 8
  // days of THAT (not 8 days from right now) -- during the preseason gap
  // like late August, the season opener can be 15+ days out, so a fixed
  // "today + 8 days" window would catch nothing at all. Anchoring to the
  // earliest real game correctly captures "this week's slate" whether
  // we're mid-season or sitting in a gap between weeks
  const upcoming = allGames
    .map((g) => ({ g, t: new Date(g.commence_time).getTime() }))
    .filter((x) => Number.isFinite(x.t) && x.t >= now);
  if (upcoming.length === 0) return { ok: false, reason: "no upcoming games found (total games in response: " + allGames.length + ")" };
  const earliest = Math.min(...upcoming.map((x) => x.t));
  const games = upcoming.filter((x) => x.t <= earliest + eightDaysMs).map((x) => x.g);
  ...
  for (const game of games) {
    const propsRes = await fetch(
      "https://api.the-odds-api.com/v4/sports/americanfootball_nfl/events/" + game.id + "/odds?apiKey=" + apiKey +
      "&regions=us&markets=player_anytime_td,player_pass_tds,player_pass_yds,player_rush_yds,player_reception_yds,player_receptions"
    );
    if (!propsRes.ok) { propsFetchFailures++; continue; }
    const eventData = await propsRes.json();
    const bookmakers = eventData.bookmakers || [];

    bookmakers.forEach((book) => {
      if (book.key !== "draftkings" && book.key !== "fanduel") return;
      (book.markets || []).forEach((market) => {
        const marketType = NFL_MARKET_KEYS[market.key];
        if (!marketType) return;
        (market.outcomes || []).forEach((outcome) => {
          const displayName = outcome.description;
          if (!displayName) return;
          const nameKey = normalizeName(displayName);
          const rowKey = marketType + "|" + nameKey;
          if (!rows[rowKey]) {
            rows[rowKey] = {
              marketType: marketType, nameKey: nameKey, name: displayName, line: null,
              dkOver: null, fdOver: null, dkUnder: null, fdUnder: null,
            };
          }
          const row = rows[rowKey];
          const american = decimalToAmerican(safeNum(outcome.price));
          if (outcome.point != null) row.line = safeNum(outcome.point);
          // anytime_td is yes/no (no Over/Under, no line) -- "Yes" goes in
          // the same "over" slot convention used for MLB home runs
          if (outcome.name === "Yes" || outcome.name === "Over") {
            if (book.key === "draftkings") row.dkOver = american; else row.fdOver = american;
          } else if (outcome.name === "Under") {
            if (book.key === "draftkings") row.dkUnder = american; else row.fdUnder = american;
          }
        });
      });
    });
  ...
  await env.DB.prepare("DELETE FROM nfl_odds").run();
  // same batching fix as MLB and WNBA's refresh functions
  const statements = Object.keys(rows).map((key) => {
    const r = rows[key];
    const bestOver = pickBest(r.dkOver, r.fdOver, null, null);
    const bestUnder = pickBest(r.dkUnder, r.fdUnder, null, null);
    return env.DB.prepare(
      "INSERT INTO nfl_odds (market_type, player_name_key, player_name, line, dk_odds_over, fd_odds_over, dk_odds_under, fd_odds_under, best_book_over, best_odds_over, best_book_under, best_odds_under, updated_at) " +
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))"
    ).bind(
      r.marketType, r.nameKey, r.name, r.line,
      r.dkOver, r.fdOver, r.dkUnder, r.fdUnder,
      bestOver.book, bestOver.odds, bestUnder.book, bestUnder.odds
    );
  });
  if (statements.length > 0) await chunkedBatch(env.DB, statements);
  return { ok: true, gamesFound: games.length, propsFetchFailures: propsFetchFailures, rowsWritten: statements.length };
}

// pulls real WNBA player prop odds from The Odds API -- a different
// provider than MLB's, used specifically because SportsGameOdds' free tier
// blocks WNBA entirely. Costs 1 request for the game list + 1 per game for
// props, so a typical 4-6 game WNBA day runs 5-7 requests per refresh --
// meaningfully tighter than MLB's budget (500/month total here, not
// 2,500), so this refreshes 1-2x/day rather than MLB's 3x
async function refreshWnbaOdds(env) {
  ...
  const gamesRes = await fetch(
    "https://api.the-odds-api.com/v4/sports/basketball_wnba/odds/?apiKey=" + apiKey + "&regions=us&markets=h2h"
  );
  if (!gamesRes.ok) {
    console.error("The Odds API games fetch failed", gamesRes.status);
    return;
  }
  ...
  for (const game of games) {
    const propsRes = await fetch(
      "https://api.the-odds-api.com/v4/sports/basketball_wnba/events/" + game.id + "/odds?apiKey=" + apiKey + "&regions=us&markets=player_points,player_rebounds,player_assists"
    );
    ...
    bookmakers.forEach((book) => {
      if (book.key !== "draftkings" && book.key !== "fanduel") return;
      (book.markets || []).forEach((market) => {
        const marketType = WNBA_MARKET_KEYS[market.key];
        if (!marketType) return;
        (market.outcomes || []).forEach((outcome) => {
          const displayName = outcome.description;
          if (!displayName) return;
  ...
  await env.DB.prepare("DELETE FROM wnba_odds").run();
  // same batching fix as MLB's refresh -- WNBA hasn't hit the timeout yet
  // given its smaller player pool, but it has the exact same underlying
  // bug (sequential awaited inserts), so fixing it now before it does
  const statements = Object.keys(rows).map((key) => {
    const r = rows[key];
    const bestOver = pickBest(r.dkOver, r.fdOver, null, null);
    const bestUnder = pickBest(r.dkUnder, r.fdUnder, null, null);
    return env.DB.prepare(
      "INSERT INTO wnba_odds (market_type, player_name_key, player_name, line, dk_odds_over, fd_odds_over, dk_odds_under, fd_odds_under, best_book_over, best_odds_over, best_book_under, best_odds_under, updated_at) " +
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))"
    ).bind(
      r.marketType, r.nameKey, r.name, r.line,
      r.dkOver, r.fdOver, r.dkUnder, r.fdUnder,
...
// only home_run is still displayed anywhere on the site as of tonight --
// strikeouts, hits+runs+rbi, total bases, and stolen bases were all
// removed from the frontend, so there's no reason to keep fetching,
// processing, and writing rows for them here. Also meaningfully cuts the
// row count this function has to write each refresh (roughly a fifth of
// what it was), which matters directly for the timeout issue below.
...
async function refreshMlbOdds(env) {
  ...
  events.forEach((event) => {
    ...
    Object.keys(odds).forEach((key) => {
      ...
      const dk = safeInt(byBookmaker.draftkings && byBookmaker.draftkings.odds);
      const fd = safeInt(byBookmaker.fanduel && byBookmaker.fanduel.odds);
      ...
      const realLine = safeNum(o.bookOverUnder) != null ? safeNum(o.bookOverUnder) : safeNum(o.fairOverUnder);
  ...
  // batched into one round-trip instead of awaiting each insert
  // sequentially -- with hundreds of rows, awaiting them one at a time was
  // very likely blowing past the Worker's execution time limit and getting
  // the whole request killed mid-flight, which is what "load failed" on
  // the client looks like from a request that never got to send a response
  const statements = Object.keys(rows).map((key) => {
    const r = rows[key];
...
export default {
  async fetch(request, env) {
    ...
    // ---- same idea as /mlb-odds, but from The Odds API instead of
    // SportsGameOdds (which blocks WNBA entirely on its free tier) -- no
    // deeplink fields here since that provider doesn't return any ----
    if (url.pathname === "/wnba-odds" && request.method === "GET") {
      const rows = await env.DB.prepare(
        "SELECT market_type, player_name_key, player_name, line, dk_odds_over, fd_odds_over, dk_odds_under, fd_odds_under, best_book_over, best_odds_over, best_book_under, best_odds_under, updated_at FROM wnba_odds"
      ).all();
      const byMarket = {};
      (rows.results || []).forEach((r) => {
        if (!byMarket[r.market_type]) byMarket[r.market_type] = {};
        byMarket[r.market_type][r.player_name_key] = {
          name: r.player_name, line: r.line,
          dkOver: r.dk_odds_over, fdOver: r.fd_odds_over,
          dkUnder: r.dk_odds_under, fdUnder: r.fd_odds_under,
          bestBookOver: r.best_book_over, bestOddsOver: r.best_odds_over,
          bestBookUnder: r.best_book_under, bestOddsUnder: r.best_odds_under,
          updatedAt: r.updated_at,
        };
      });
      return json({ ok: true, markets: byMarket });
    ...
    // ---- same idea, third provider setup -- also The Odds API, same as
    // WNBA, since SportsGameOdds' NFL data was confirmed to be team-level
    // only (no real player props at all) ----
    if (url.pathname === "/nfl-odds" && request.method === "GET") {
      const rows = await env.DB.prepare(
        "SELECT market_type, player_name_key, player_name, line, dk_odds_over, fd_odds_over, dk_odds_under, fd_odds_under, best_book_over, best_odds_over, best_book_under, best_odds_under, updated_at FROM nfl_odds"
      ).all();
      const byMarket = {};
      (rows.results || []).forEach((r) => {
        if (!byMarket[r.market_type]) byMarket[r.market_type] = {};
    ...
    if (url.pathname === "/admin/refresh-wnba-odds" && request.method === "POST") {
      await refreshWnbaOdds(env);
      return json({ ok: true, message: "Refreshed." });
    }

    if (url.pathname === "/admin/refresh-nfl-odds" && request.method === "POST") {
      const result = await refreshNflOdds(env);
      return json(Object.assign({ message: result && result.ok ? "Refreshed." : "Refresh ran but found a problem." }, result));
    }

    return json({ error: "Not found" }, 404);
  },

  // three separate providers/keys, each with its own budget:
  // - MLB: SportsGameOdds, 3x/day (bigger object budget there)
  // - WNBA (and NBA once built): one dedicated The Odds API account, 2x/day
  // - NFL: its own separate The Odds API account (different email), so it
  //   never competes with WNBA/NBA for the same 500/month cap. Runs 6x/week,
  //   skipping Tuesdays specifically since the NFL never plays that day --
  //   real margin under budget (439/month) instead of the 566 it would hit
  //   sharing a key with WNBA
  async scheduled(event, env, ctx) {
    if (event.cron === "0 12,16,20 * * *") {
      ctx.waitUntil(refreshMlbOdds(env));
    } else if (event.cron === "0 14,22 * * *") {
      ctx.waitUntil(refreshWnbaOdds(env));
    } else if (event.cron === "0 12 * * SUN,MON,WED,THU,FRI,SAT") {
      ctx.waitUntil(refreshNflOdds(env));
    }
  },
};
