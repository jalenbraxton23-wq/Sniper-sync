...
export default {
  async fetch(request, env) {
    ...
    // ---- get the full WNBA track record for this user ----
    if (url.pathname === "/wnba-tracker" && request.method === "GET") {
      const rows = await env.DB.prepare("SELECT date_key, data FROM wnba_pick_log WHERE user_id = ?").bind(userId).all();
      const log = {};
