...
function pickBest(dk, fd) {
  const dkDecimal = americanToDecimal(dk);
  const fdDecimal = americanToDecimal(fd);
  if (dkDecimal == null && fdDecimal == null) return { book: null, odds: null };
  if (dkDecimal != null && (fdDecimal == null || dkDecimal >= fdDecimal)) return { book: "draftkings", odds: dk };
  return { book: "fanduel", odds: fd };
}
...
async function refreshMlbOdds(env) {
    ...
    Object.keys(odds).forEach((key) => {
      ...
      if (!rows[rowKey]) {
        rows[rowKey] = {
          ...
          betType: o.betTypeID, line: o.line != null ? Number(o.line) : null,
      ...
      if (o.betTypeID === "yn" && o.sideID === "yes") {
        row.dkOver = dk; row.fdOver = fd;
      } else if (o.betTypeID === "ou" && o.sideID === "over") {
        row.dkOver = dk; row.fdOver = fd; if (o.line != null) row.line = Number(o.line);
      } else if (o.betTypeID === "ou" && o.sideID === "under") {
        row.dkUnder = dk; row.fdUnder = fd; if (o.line != null) row.line = Number(o.line);
      }
    });
  });
  ...
  for (const key of Object.keys(rows)) {
    const r = rows[key];
    const bestOver = pickBest(r.dkOver, r.fdOver);
    const bestUnder = pickBest(r.dkUnder, r.fdUnder);
    await env.DB.prepare(
      "INSERT INTO mlb_odds (market_type, player_name_key, player_name, bet_type, line, dk_odds_over, fd_odds_over, dk_odds_under, fd_odds_under, best_book_over, best_odds_over, best_book_under, best_odds_under, updated_at) " +
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))"
    ).bind(
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/mlb-odds" && request.method === "GET") {
      const rows = await env.DB.prepare(
        "SELECT market_type, player_name_key, player_name, bet_type, line, dk_odds_over, fd_odds_over, dk_odds_under, fd_odds_under, best_book_over, best_odds_over, best_book_under, best_odds_under, updated_at FROM mlb_odds"
      ).all();
      const byMarket = {};
